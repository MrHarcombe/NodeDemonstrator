...
class UsageControlsFrame(ttk.Frame):
    ...
    def __init__(self, parent, canvas_frame):
        ...
        self.__algochoice = ttk.StringVar(
        ...
        algo_combo = ttk.Combobox(
            self, values=[], textvariable=self.__algochoice, state="readonly"
        )
        ...
        self.__step_button = ttk.Button(
            self, text="Step Trace", command=self.__trace_step
        )
        self.__step_button.grid(sticky=tk.NSEW, pady=(15, 3))
        self.__timed_button = ttk.Button(
            self, text="Timed Trace", command=self.__trace_timed
        )
    ...
    def __create_trace_frame(self):
        ...
        if not all((from_good, to_good)):
            ...
            if message is not None:
                dialogs.Messagebox.show_error(title="Unknown Node", message=message)
                return
        ...
        match self.__algochoice.get():
            ...
            case "Pre Order Traversal":
                if from_given:
                    self.__trace_frame = TreeTraversalFrame(
                        self, self.__canvas_frame, "Pre", from_node, to_node
                    )
                else:
                    dialogs.Messagebox.show_error(
                        title="Required Node",
            ...
            case "In Order Traversal":
                if from_given:
                    self.__trace_frame = TreeTraversalFrame(
                        self, self.__canvas_frame, "In", from_node, to_node
                    )
                else:
                    dialogs.Messagebox.show_error(
                        title="Required Node",
            ...
            case "Post Order Traversal":
                if from_given:
                    self.__trace_frame = TreeTraversalFrame(
                        self, self.__canvas_frame, "Post", from_node, to_node
                    )
                else:
                    dialogs.Messagebox.show_error(
                        title="Required Node",
